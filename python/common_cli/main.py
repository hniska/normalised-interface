...
class RunAction(Action):
    ...
    def cb_action(self, uinfo, name, kp, input, output):
        ...
        device_type = GetDeviceType(device, uinfo.username, uinfo.context)
        if device_type == 'tailf-ned-cisco-ios':
            interfaces = GetCiscoiosInterface(device, uinfo.username, uinfo.context)
        elif device_type == 'junos-rpc':
            interfaces = GetJunosInterface(device, uinfo.username, uinfo.context)
        else:
            interfaces = GetTestInterface(device, uinfo.username, uinfo.context)
...
def GetCiscoiosInterface(device_name, username, context):
...
def GetJunosInterface(device_name, username, context):
...
def GetTestInterface(device_name, username, context):
...
def GetDeviceType(device_name, username, context):
    with ncs.maapi.single_read_trans(username, context) as trans:
        root = ncs.maagic.get_root(trans)
...
class InterfacesHandler(object):
    ...
    def get_data(self, tctx, device_name):
        ...
        if device_name not in self.cache:
            device_type = GetDeviceType(device_name, tctx.uinfo.username, tctx.uinfo.context)
            if device_type == 'tailf-ned-cisco-ios':
                interfaces = GetCiscoiosInterface(device_name, tctx.uinfo.username, tctx.uinfo.context)
                self.cache[device_name] = interfaces
            elif device_type == 'junos-rpc':
                interfaces = GetJunosInterface(device_name, tctx.uinfo.username, tctx.uinfo.context)
                self.cache[device_name] = interfaces
            else:
                interfaces = GetTestInterface(device_name, tctx.uinfo.username, tctx.uinfo.context)
                self.cache[device_name] = interfaces
            return interfaces
        else:
...
class Main(ncs.application.Application):
    def setup(self):
        ...
        def start_dcb_fun(state):
            ...
            dcb.register("/ncs:devices/ncs:device/norm-intf:interfaces/norm-intf:interface",
                InterfacesHandler())
